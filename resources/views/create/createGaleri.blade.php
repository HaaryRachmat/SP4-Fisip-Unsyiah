      ...
      <div class="relative bg-white p-6 shadow-lg rounded-md text-gray-900 z-20" x-show="showModal1"
        ...
        <main class="p-2 text-center">
          <div class="">
            ...
            <input class="w-full px-5 py-1 border border-gray-400  text-gray-700 bg-white rounded" type="text"
              required="" wire:model="judul" placeholder="Judul" aria-label="Judul">
          ...
          <div class="mt-2">
            ...
            <input class="w-full px-5  py-1 border border-gray-400 text-gray-700 bg-white rounded" type="text"
              required="" wire:model="tanggal" placeholder="cth. 03 Juli 2021" aria-label="Tanggal">
          ...
          <div class="mt-2">
            ...
            <textarea class="w-full px-5  py-1 border border-gray-400 text-gray-700 bg-white rounded" type="text"
              required="" wire:model="deskripsi" placeholder="Deskripsi disini" aria-label="Deskripsi"></textarea>
            @error ('deskripsi') <span class="text-red-500 flex justify-start">{{ $message }}</span> @enderror
          </div>

          <div class="mt-2">

            <label class="flex justify-start text-lg font-light text-gray-800" for="jenis">Gambar Galeri</label>
            <input class="w-full px-5  py-1 text-gray-700 bg-white rounded" type="file" required=""
              wire:model="additionalPhotos" placeholder="Gambar Galeri" name="additional_photos[]"
              aria-label="additional_photos" multiple />
            @error ('additionalphotos.*') <span class="text-red-500 flex justify-start">{{ $message }}</span> @enderror

            <!-- Notifikasi succes akan tampil saat gambar berhasil dipilih -->
            @if ( $additionalPhotos == null)
            @else
            <div
              class='flex items-center text-white text-sm max-w-sm w-full bg-green-500 shadow-md rounded-lg overflow-hidden mx-auto'>
              ...
              <div class='flex items-center px-2 py-3'>
                <div class='mx-3'>
                  {{-- <p>{{ $file_name }}</p> --}}
                  <p>File Gambar telah di Upload</p>
                </div>
              </div>
            </div>
            @endif
            ...
            <label class="flex justify-start text-lg font-light text-gray-800" for="jenis">Thumbnail</label>
            <input class="w-full px-5  py-1 text-gray-700 bg-white rounded" type="file" required=""
              wire:model="file_name" placeholder="Gambar Galeri" name="file_name" aria-label="file_name" />
            @error ('file_name') <span class="text-red-500 flex justify-start">{{ $message }}</span> @enderror
          </div>
          @php
          $array = explode(".",$file_name);
          @endphp
          @if ($array[count($array)-1] == "png" || $array[count($array)-1] == "jpg" || $array[count($array)-1] ==
          "jpeg")
          <div class="mb-4">
          ...
          <div
            ...
            <div class='flex items-center px-2 py-3'>
              <div class='mx-3'>
                {{-- <p>{{ $file_name }}</p> --}}
                <p>File Thumbnail telah di Upload</p>
              </div>
            </div>
          </div>
          @endif
        ...
        <footer class="flex justify-end p-2">
          ...
          <button
            class="bg-blue-600 font-semibold text-white p-2 w-32 ml-2 rounded-full hover:bg-blue-700 focus:outline-none focus:ring shadow-lg hover:shadow-none transition-all duration-300"
            wire:click="store()">
            Simpan
          </button>
        </footer>
